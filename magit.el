...
(defvar magit-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "<backspace>") 'magit-show-item-or-scroll-down)
    ...
    (define-key map (kbd "M-l") 'magit-key-mode-popup-logging)
...
\\{magit-log-edit-mode-map}"
  ...
  (cond ((magit-rebase-info)
	...
	(t
	 ...
	 (with-current-buffer (magit-get-log-edit-buffer)
	   ;; So, this should be SHOWN to the user. It does contain
	   ;; stuff, that magit cannot know!
	   (when (file-exists-p ".git/MERGE_MSG")
	     (erase-buffer)
	 ...
	 (magit-pop-to-log-edit-half "commit")))))
